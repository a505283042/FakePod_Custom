...
static bool has_avi_extension(const char *name)
{
    ...
    return ext[0] == '.' &&
        ascii_tolower(static_cast<unsigned char>(ext[1])) == 'a' &&
        ascii_tolower(static_cast<unsigned char>(ext[2])) == 'v' &&
        ascii_tolower(static_cast<unsigned char>(ext[3])) == 'i';
}
...
static const char *pool_string(const char *pool, size_t pool_size, uint32_t offset)
{
    ...
    return memchr(text, '\0', pool_size - static_cast<size_t>(offset)) != nullptr ? text : nullptr;
}

static int entry_compare(const EntryIndex &lhs, const EntryIndex &rhs, const char *pool, size_t pool_size)
{
...
static bool reserve_entries(DirectoryScanSession *session, size_t needed)
{
    ...
    size_t next = session->entry_capacity > 0U ? session->entry_capacity : kInitialEntryCapacity;
    while (next < needed) {
        if (next > SIZE_MAX / 2U) { next = needed; break; }
        next *= 2U;
    }
    if (next > SIZE_MAX / sizeof(EntryIndex)) return false;
    void *block = session->entries;
    if (!grow_psram_block(
            &block,
            session->entry_capacity * sizeof(EntryIndex),
            next * sizeof(EntryIndex))) return false;
    session->entries = static_cast<EntryIndex *>(block);
    session->entry_capacity = next;
    return true;
}
...
static bool reserve_pool(DirectoryScanSession *session, size_t needed)
{
    ...
    size_t next = session->pool_capacity > 0U ? session->pool_capacity : kInitialPoolCapacity;
    while (next < needed) {
        if (next > SIZE_MAX / 2U) { next = needed; break; }
        next *= 2U;
    }
    void *block = session->string_pool;
    if (!grow_psram_block(&block, session->pool_capacity, next)) return false;
    session->string_pool = static_cast<char *>(block);
    session->pool_capacity = next;
    return true;
}

static esp_err_t append_entry(DirectoryScanSession *session, const char *name, bool is_dir, uint64_t size_bytes)
{
    ...
    if (session->pool_size > UINT32_MAX ||
        name_bytes > static_cast<size_t>(UINT32_MAX) - session->pool_size) return ESP_ERR_INVALID_SIZE;
    if (!reserve_entries(session, session->count + 1U) ||
        !reserve_pool(session, session->pool_size + name_bytes)) return ESP_ERR_NO_MEM;
    ...
    entry.size_bytes = is_dir ? 0U : size_bytes;
    entry.name_off = name_off;
    entry.flags = is_dir ? kEntryDirectory : kEntryNone;
    return ESP_OK;
}

static void heap_sift_down(EntryIndex *entries, size_t count, size_t root, const char *pool, size_t pool_size)
{
    while (true) {
        ...
        if (right < count && entry_compare(entries[largest], entries[right], pool, pool_size) < 0) largest = right;
...
{
    if (entries == nullptr || count < 2U) return;
    for (size_t i = count / 2U; i > 0U; --i) heap_sift_down(entries, count, i - 1U, pool, pool_size);
...
esp_err_t join_child_path(const char *parent, const char *name, char *out, size_t out_size)
{
    if (parent == nullptr || name == nullptr || out == nullptr || out_size == 0U ||
        !path_is_inside_root(parent) || name[0] == '\0' || strchr(name, '/') != nullptr) return ESP_ERR_INVALID_ARG;
...
{
    if (path == nullptr || out == nullptr || out_size == 0U || !path_is_inside_root(path)) return ESP_ERR_INVALID_ARG;
...
esp_err_t begin_directory_scan(const char *path, DirectoryScanSession *session)
{
    ...
    char *path_copy = static_cast<char *>(heap_caps_calloc(kPathBytes, 1U, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    char *scratch = static_cast<char *>(heap_caps_malloc(kEntryNameBytes + kPathBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    ...
    if (!reserve_entries(session, kInitialEntryCapacity) || !reserve_pool(session, kInitialPoolCapacity)) {
        cancel_directory_scan(session);
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t scan_directory_step(DirectoryScanSession *session, size_t max_raw_entries, bool *out_done)
{
    if (session == nullptr || out_done == nullptr || session->dir == nullptr ||
        ...
        session->path == nullptr || session->scratch == nullptr || max_raw_entries == 0U) return ESP_ERR_INVALID_ARG;
    ...
    *out_done = false;
    ...
    for (size_t processed = 0; processed < max_raw_entries; ++processed) {
        ...
        if (end) { *out_done = true; return ESP_OK; }
        if (name_is_hidden(name)) { taskYIELD(); continue; }

        if (join_child_path(session->path, name, full_path, kPathBytes) != ESP_OK) { taskYIELD(); continue; }
        struct stat info = {};
        {
            StorageSdLockGuard guard(kStorageLockTimeout);
            if (!guard) return ESP_ERR_TIMEOUT;
            if (stat(full_path, &info) != 0) { taskYIELD(); continue; }
        }

        const bool is_dir = S_ISDIR(info.st_mode);
        if (!is_dir && (!S_ISREG(info.st_mode) || !has_avi_extension(name))) { taskYIELD(); continue; }
        const uint64_t size_bytes = is_dir ? 0U : static_cast<uint64_t>(info.st_size < 0 ? 0 : info.st_size);
        const esp_err_t ret = append_entry(session, name, is_dir, size_bytes);
...
esp_err_t finish_directory_scan(DirectoryScanSession *session, DirectorySnapshot *out_snapshot)
{
    if (session == nullptr || out_snapshot == nullptr || session->dir == nullptr ||
        session->entries == nullptr || session->string_pool == nullptr) return ESP_ERR_INVALID_ARG;
    {
        StorageSdLockGuard guard(portMAX_DELAY);
        if (!guard) return ESP_ERR_TIMEOUT;
...
{
    if (snapshot == nullptr || snapshot->entries == nullptr || snapshot->string_pool == nullptr || index >= snapshot->count) return nullptr;
    const EntryIndex *entry = &snapshot->entries[index];
    return pool_string(snapshot->string_pool, snapshot->pool_size, entry->name_off) != nullptr ? entry : nullptr;
}

const char *entry_name(const DirectorySnapshot *snapshot, size_t index)
{
    const EntryIndex *entry = entry_at(snapshot, index);
    return entry != nullptr ? pool_string(snapshot->string_pool, snapshot->pool_size, entry->name_off) : nullptr;
}

bool entry_is_directory(const EntryIndex *entry)
{
    return entry != nullptr && index_is_directory(*entry);
}
