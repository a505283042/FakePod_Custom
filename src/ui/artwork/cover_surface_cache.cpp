...
static bool cover_cache_insert(const CoverRequest &request, uint8_t *normal)
{
...
static bool cover_psram_budget_ok(size_t source_bytes)
{
    ...
    const size_t need = source_bytes + COVER_SURFACE_BYTES + COVER_PSRAM_SAFETY_RESERVE_BYTES;
...
{
    if (out_normal == nullptr || out_source_width == nullptr || out_source_height == nullptr) {
        return false;
    }
    *out_normal = nullptr;
    *out_source_width = 0U;
...
static void cover_task_main(void *)
{
    ...
    ESP_LOGI(TAG, "封面最终表面服务已启动：%dx%d RGB565 x1 + Overlay alpha，任务优先级=%u core=%ld",
        FAKEPOD_LCD_WIDTH, FAKEPOD_LCD_HEIGHT, static_cast<unsigned>(COVER_TASK_PRIORITY),
        static_cast<long>(COVER_TASK_CORE));
    ...
    while (true) {
        ...
        const bool rendered = cover_render_surface(
            compressed, &normal, &source_width, &source_height, &render_stats);
        ...
        if (!cover_cache_insert(request, normal)) {
            heap_caps_free(normal);
        ...
        ESP_LOGI(TAG, "封面预处理完成：track=%lu format=%s source=%ux%u -> %dx%d，总计=%lums 解码=%lums 采样=%lums core=%ld stack_hwm=%u",
